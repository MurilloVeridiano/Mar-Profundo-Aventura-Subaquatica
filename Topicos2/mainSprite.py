...
class Peixe(pygame.sprite.Sprite):
    def __init__(self):
        super().__init__()
        self.sprites = []
        # Adicionar todos os sprites para a animação
        self.sprites.append(pygame.image.load('Imagens/peixe.png'))
        self.atual = 0  # Índice do sprite atual
        self.image = self.sprites[self.atual]
        self.rect = self.image.get_rect()
        self.frame_rate = 10  # Quantos quadros por segundo para a animação
        self.frame_count = 0  # Contador para alternar os sprites
        self.velocidade = 5  # Velocidade de movimento

    def update(self):
        # Alternar entre os sprites para a animação
        self.frame_count += 1
